...
void Read_Data(char file_name[], canh a[], int *n){	
	FILE *f;
	f=fopen(file_name, "r");
	if (f==NULL){
		printf("Loi mo file!!!\n");
...
void Print_Data(canh a[], int m, int la_PA){
	int i;
	float tong=0.0;
	for(i=0; i<m; i++){
		printf("%3d %c%c=%8.2f\n", i+1, a[i].dau+97, a[i].cuoi+97, a[i].do_dai);
		//+97 la de ra chu cai, %c kia
		if (la_PA)
			tong+= a[i].do_dai;
	}
	if (la_PA)
...
int dinh_cap3(canh PA[], int k, canh moi){
	int i=0, dem_dau=1,dem_cuoi=1;
	while(i<k && dem_dau<3&&dem_cuoi<3){
...
int main(){
	...
	Read_Data("TSP.txt", ds_canh, &n);
	printf("Danh sach cac canh cua do thi:\n");
	Print_Data(ds_canh, n*(n-1)/2, 0);
